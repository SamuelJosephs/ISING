...
# Compiler settings
FC = gfortran
FCFLAGS =  -I/usr/include -lfftw3 -Wno-line-truncation -fopenmp -O3
MODDIR = -J./obj
INCDIR = -I./obj 
...
SOURCES = $(SRCDIR)/rand.f90 $(SRCDIR)/CubePartition.f90 $(SRCDIR)/ISING.f90 $(SRCDIR)/RGFlow.f90 $(SRCDIR)/atom.f90 $(SRCDIR)/chainMeshCell.f90 $(SRCDIR)/chainMesh.f90 $(SRCDIR)/energyMin.f90 $(SRCDIR)/vecNd.f90 $(SRCDIR)/LLG.f90 $(SRCDIR)/StereographicProjection.f90 $(SRCDIR)/constants.f90 $(SRCDIR)/reciprocalSpaceProcesses.f90 
...
$(OBJDIR)/reciprocalSpaceProcesses.0: $(OBJDIR)/chainMesh.o $(OBJDIR)/vecNd.o
# Clean target
clean:
